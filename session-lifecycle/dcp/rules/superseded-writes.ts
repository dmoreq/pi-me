...
export const supersededWritesRule: PruneRule = {
	...
	prepare(msg, ctx) {
		...
		if (filePath) {
			...
			if (ctx.config.debug) {
				getLogger().debug(`SupersededWrites: found file operation at index ${ctx.index}: ${filePath}`);
			}
		}
	},
	...
	process(msg, ctx) {
		...
		if (laterWrite) {
			...
			if (ctx.config.debug) {
				getLogger().debug(`SupersededWrites: marking superseded write at index ${ctx.index}: ${msg.metadata.filePath}`);
			}
		}
	},
};
