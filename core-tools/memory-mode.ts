...
/**
 * Composite memory dialog: instruction text input + location selector + preview.
 */
class MemoryDialog {
  private tui: TUI;
  private theme: Theme;
  ...
  private done: (result: { instruction: string; locationIndex: number } | null) => void;

  constructor(
    tui: TUI,
    theme: Theme,
    existingInstruction: string,
    done: (result: { instruction: string; locationIndex: number } | null) => void,
  ) {
    this.tui = tui;
    this.theme = theme;
    this.instruction = existingInstruction;
    this.selectedLocation = 0;
    this.editMode = existingInstruction.length === 0;
    this.done = done;
  }
  ...
  handleInput(data: string): void {
    ...
    if (matchesKey(data, Key.enter)) {
      ...
      this.done({ instruction: this.instruction.trim(), locationIndex: this.selectedLocation });
      return;
    }

    if (matchesKey(data, Key.up) || matchesKey(data, Key.down)) {
      // Typing when no instruction yet: navigate locations
      if (this.editMode && this.instruction.length === 0 && data === Key.down) {
        this.selectedLocation = Math.min(2, this.selectedLocation + 1);
        this.tui.requestRender();
        return;
      }
      if (this.editMode && this.instruction.length === 0 && data === Key.up) {
        this.selectedLocation = Math.max(0, this.selectedLocation - 1);
        this.tui.requestRender();
        return;
      }
      // Navigate locations
    ...
    if (this.editMode) {
      // Handle backspace
      if (data === "\x7f" || data === "\b") {
        this.instruction = this.instruction.slice(0, -1);
        this.tui.requestRender();
        return;
      }
      // Tab: switch to location selection mode
      if (data === "\t") {
        this.editMode = false;
        this.tui.requestRender();
        return;
  ...
  render(width: number): string[] {
    ...
    lines.push("");
    lines.push(truncateToWidth(t.fg("accent", t.bold("  Save Memory Instruction")), width));
    lines.push(truncateToWidth(t.fg("dim", "  ─" + "─".repeat(Math.min(60, width - 4))), width));
    lines.push("");

    // Instruction text area
    lines.push(truncateToWidth(t.fg("accent", "  Instruction:"), width));
    const displayText = this.instruction.length > 0
      ? this.instruction
      : t.fg("dim", "e.g., never use git commands");
    const cursor = this.editMode ? t.fg("accent", "█") : " ";
    lines.push(truncateToWidth(`  ${displayText}${cursor}`, width));
    lines.push("");

    // Location selector
    lines.push(truncateToWidth(t.fg("accent", "  Save to:"), width));
    const locations = [
      { label: "Project Local", desc: "AGENTS.local.md (gitignored, just for you)" },
      { label: "Project", desc: "AGENTS.md (shared with team)" },
      { label: "Global", desc: "~/.pi/agent/AGENTS.md (all your projects)" },
    ];
    for (let i = 0; i < locations.length; i++) {
      const selected = i === this.selectedLocation;
      const prefix = selected ? t.fg("accent", "  ● ") : "  ○ ";
      const label = selected ? t.fg("accent", locations[i].label) : locations[i].label;
      const desc = t.fg("dim", ` — ${locations[i].desc}`);
      const suffix = !this.editMode && selected ? " " + t.fg("warning", "◄") : "";
      lines.push(truncateToWidth(`${prefix}${label}${desc}${suffix}`, width));
    }
    lines.push("");

    // Footer
    lines.push(truncateToWidth(t.fg("dim", "  ↑/↓ to navigate · Tab to switch · Enter to confirm · Esc to cancel"), width));
    lines.push("");
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("mem", {
		...
		handler: async (_args, ctx) => {
			// Use the composite memory dialog (input + location in one step)
			const result = await ctx.ui.custom<{ instruction: string; locationIndex: number } | null>(
				(tui, theme, _kb, done) => new MemoryDialog(tui, theme, "", done),
			...
			await handleMemoryInstruction(pi, ctx, result.instruction, result.locationIndex);
		},
	});
...
async function handleMemoryInstruction(pi: ExtensionAPI, ctx: ExtensionContext, instruction: string, locationIndex: number): Promise<void> {
	const cwd = ctx.cwd;
